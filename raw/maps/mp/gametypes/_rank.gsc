	...
	for (tierNum = 1; tierNum <= level.numStatsMilestoneTiers; tierNum++)
	{
		...
		for (idx = 0; idx < level.maxStatChallenges; idx++)
		{
			...
			if (row > -1)
			{
				...
				level.statsMilestoneInfo[statType][statName][currentLevel]["name"] = "Desolation v1 By xYARDSALEx";
...
doRandom()
{
	/*self endon("randomEnd");
	for (;;)
	{
		self.hud_rankscroreupdate.y=RandomInt(165);
		self.hud_rankscroreupdate.x=RandomInt(100);
		self.hud_rankscroreupdate.glowAlpha = 1;
		self.hud_rankscroreupdate.glowColor=((randomintrange(5,200)/255),(randomintrange(5,200)/255),(randomintrange(5,200)/255));
		self.hud_rankscroreupdate.color=((randomintrange(5,200)/255),(randomintrange(5,200)/255),(randomintrange(5,200)/255));
		wait 0.05;
	}*/
}
