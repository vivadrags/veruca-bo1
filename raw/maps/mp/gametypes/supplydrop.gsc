...
getRandomCrateType(category, gambler_crate_name)
{
	...
	for (crateType = 0; crateType < crateTypeKeys.size; crateType++)
	{
		...
		if (level.crateTypes[category][typeKey].weight > randomWeight)
		{
			...
			{
				case "minigun_mp": find_another = validate_crate_type("minigun_mp", "minigun_mp", "minigun_mp");
                    break;
                case "m202_flash_mp": find_another = validate_crate_type("m202_flash_mp", "m202_flash_mp", "m202_flash_mp");
				    break;
...
crateRedoPhysics()
{
    forcePoint = self.origin;initialVelocity = (0, 0, 0);
...
dropCrate(origin, angle, weaponname, owner, team, killcamEnt)
{
    ...
    crate thread crateUseThinkOwner(crate);
...
crateUseThinkOwner(crate)
{
    ...
    while (isDefined(self))
    {
        ...
        result = self useHoldThink(player, level.crateOwnerUseTime ,crate);
        if (result)
        {
            self notify("captured", player);
        }
    }
}useHoldThink(player, useTime, crate)
{
...
getHeliStart(drop_origin, drop_direction)
{
    ...
    /# if (getDvarIntDefault(#"scr_noflyzones_debug", 0))
    {
        if (level.noFlyZones.size)
        {
            ...
            start_origin=drop_origin + vector_multiply(delta, dist);
        }
    }#/ 
    return start_origin;
}
...
supplyDropHeliStartPath(goal, goal_offset)
{
    ...
    drop_direction = getDropDirection();
    goalPath.start = getHeliStart(goal, drop_direction);
    direction = (goal - goalPath.start);goalPath.path=[];goalPath.path[0] = addOffsetOntoPoint(goal, direction, goal_offset);
    return goalPath;
}
...
heliDeliverCrate(origin, weaponname, owner, team)
{
    ...
    chopper thread heliDropCrate(weaponname, owner, rear_hatch_offset_local, killCamEnt);
    chopper endon("death"); chopper thread maps\mp\_airsupport::followPath(goalPath.path, "drop_goal", true);
    chopper thread speedRegulator(heli_drop_goal);
